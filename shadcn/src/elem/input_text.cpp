...
using shadcn::InputText;

std::unordered_map< std::string, InputText::InputTextState > InputText::state;
...
bool InputText::inputEx( const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, const InputTextData& data, ImGuiInputTextCallback callback, void* callback_user_data )
{
    ...
    const auto font = shadcn::g->fonts->get( data.fontVariant );

    const ImVec2 label_size = font.calculateSize( label );
    ...
    } else if ( ( init_state && g.ActiveId != id ) || init_changed_specs )
    {
        ...
        if ( flags & ImGuiInputTextFlags_ElideLeft )
            state->Scroll.x += ImMax( 0.0f, font.calculateSize( buf ).x - frame_size.x + style.FramePadding.x * 2.0f );
    ...
    if ( g.ActiveId == id )
    {
        ...
        const float mouse_y = ( is_multiline ? ( io.MousePos.y - draw_window->DC.CursorPos.y ) : ( font.data.second * 0.5f ) );
    ...
    if ( g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id )
    {
        ...
        const int row_count_per_page = ImMax( ( int ) ( ( inner_size.y - style.FramePadding.y ) / font.data.second ), 1 );
        ...
        {
            if ( io.KeyCtrl )
                SetScrollY( draw_window, ImMax( draw_window->Scroll.y - font.data.second, 0.0f ) );
            else
        ...
        {
            if ( io.KeyCtrl )
                SetScrollY( draw_window, ImMin( draw_window->Scroll.y + font.data.second, GetScrollMaxY( ) ) );
            else
        ...
        } else if ( IsKeyPressed( ImGuiKey_PageUp ) && is_multiline )
        {
            ...
            scroll_y -= row_count_per_page * font.data.second;
    ...
    if ( !is_multiline )
    {
        ...
        {
            outline = colorPalette[ "border" ].value;
            text = colorPalette[ "text_on_primary" ].value;
            ring = colorPalette[ "ring" ].value;
        } else
        {
            outline = colorPalette[ "destructive" ].modulate( 0.20f );
            text = colorPalette[ "text_destructive" ].value;
            ring = colorPalette[ "ring_destructive" ].value;
            st.focused = true;
        }

        st.border = ImLerp( st.border, st.focused ? ring.Value : outline.Value, g.IO.DeltaTime * 20.f );
        st.focus = ImLerp( st.focus, st.focused ? Color( ring ).modulate( 0.5f * style.Alpha ).Value : Color( ring ).modulate( 0 ).Value, g.IO.DeltaTime * 20.f );

        window->DrawList->AddRect( frame_bb.Min - ImVec2( 1, 1 ), frame_bb.Max + ImVec2( 1, 1 ), Color( st.focus ).modulate( ), data.rounding, 0, 4 );

        // window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, Color( bg ).modulate( ), 4 );
        window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, Color( st.border ).modulate( ), data.rounding );
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + data.pad;
    ...
    if ( render_cursor || render_selection )
    {
        ...
        if ( render_cursor && state->CursorFollow )
        {
            ...
            if ( is_multiline )
            {
                ...
                if ( cursor_offset.y - font.data.second < scroll_y )
                    scroll_y = ImMax( 0.0f, cursor_offset.y - font.data.second );
        ...
        if ( render_selection )
        {
            ...
            ImColor bg_color = colorPalette[ "primary" ].modulate( ); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            {
                if ( rect_pos.y > clip_rect.w + font.data.second )
                    break;
                ...
                } else
                {
                    ...
                    ImRect rect( rect_pos + ImVec2( 0.0f, bg_offy_up - font.data.second ), rect_pos + ImVec2( rect_size.x, bg_offy_dn ) );
                ...
                rect_pos.y += font.data.second;
            }

            if ( is_multiline || ( buf_display_end - buf_display ) < buf_display_max_length )
            {
                ImColor col = is_displaying_hint ? colorPalette[ "text_muted" ].modulate( ) : colorPalette[ "text" ].modulate( );
                draw_window->DrawList->AddText( font.data.first, font.data.second, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect );

                if ( clip.GetSize( ).x > 0 )
                {
                    draw_window->DrawList->PushClipRect( clip.Min, clip.Max, 1 );
                    draw_window->DrawList->AddText( font.data.first, font.data.second, draw_pos - draw_scroll, colorPalette[ "primary_foreground" ].modulate( ), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect );
                    draw_window->DrawList->PopClipRect( );
                }
            }
        }
        ...
        if ( ( is_multiline || ( buf_display_end - buf_display ) < buf_display_max_length ) && !render_selection )
        {
            ImColor col = is_displaying_hint ? colorPalette[ "text_muted" ].modulate( ) : colorPalette[ "text" ].modulate( );
            draw_window->DrawList->AddText( font.data.first, font.data.second, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect );
        }
        ...
        if ( render_cursor )
        {
            ...
            ImRect cursor_screen_rect( cursor_screen_pos.x, cursor_screen_pos.y - font.data.second + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f );
            ...
            if ( !is_readonly )
            {
                ...
                g.PlatformImeData.InputPos = ImVec2( cursor_screen_pos.x - 1.0f, cursor_screen_pos.y - font.data.second );
                g.PlatformImeData.InputLineHeight = font.data.second;
            }
        }
    } else
    {
        ...
        if ( is_multiline )
            text_size = ImVec2( inner_size.x, InputTextCalcTextLenAndLineCount( buf_display, &buf_display_end ) * font.data.second ); // We don't need width
        ...
        if ( is_multiline || ( buf_display_end - buf_display ) < buf_display_max_length )
        {
            ...
            if ( flags & ImGuiInputTextFlags_ElideLeft )
                draw_pos.x = ImMin( draw_pos.x, frame_bb.Max.x - font.calculateSize( buf_display ).x - style.FramePadding.x );
...
InputText::InputText( const std::string& label, const std::string& hint, char* buf, size_t buf_size, const InputTextData& data )
{
    inputEx( label.c_str( ), hint.c_str( ), buf, static_cast< int >( buf_size ), data.size, data.imFlags, data, nullptr, nullptr );
}
